...
Error received
  │
  ├── HTTP 402 ──▶ Not an error. Extract payment challenge and proceed with payment.
  │
  ├── HTTP 409 ──▶ On POST /api/v1/pay: payment already exists. Treat as success.
  │                On other endpoints: fix your idempotency key and retry.
  │
  ├── HTTP 429 / 500 / 503 ──▶ Safe to retry with exponential backoff.
  │
  ├── HTTP 401 (code 10004) ──▶ Refresh timestamp + nonce, re-sign, retry once.
  │
  └── HTTP 400 / 403 / 404 ──▶ Do NOT retry. Fix the root cause first.
```
...
Client                              Server
  ...
  ├───────────────────────────────────►│
  │                                    ├─ 1. Is |timestamp - now| ≤ 5 min?
  │                                    │    No → 10004, reject
  │                                    │
  │                                    ├─ 2. Has this nonce been seen before?
  │                                    │    Yes → 10004, reject
  │                                    │    (nonces cached in Redis for window duration)
  │                                    │
  │                                    ├─ 3. Verify Ed25519 signature
  │                                    │    Fail → 10004, reject
  │                                    │
  │                                    └─ 4. All checks passed → process request
```
